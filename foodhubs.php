...
if ($conn) {
    ...
    if ($stmtFood) {
        while ($row = sqlsrv_fetch_array($stmtFood, SQLSRV_FETCH_ASSOC)) {
            $foodHubs[] = $row;
        }
    }
}
        ...
        <div class="info-card">
            ...
            <?php if (empty($foodHubs)): ?>
                <div class="text-center py-4">
                    ...
                    <p class="text-muted mt-2 small">No food hubs found on campus.</p>
                </div>
            <?php else: ?>
                <div class="events-list">
                    <?php foreach ($foodHubs as $fh): ?>
                    <div class="event-item" onclick="focusLocation('<?= htmlspecialchars($fh['NAME']) ?>')">
                        <div class="text-warning fw-bold mb-1" style="font-size: 0.65rem;"><i class="bi bi-star-fill me-1"></i>Dining & Snacks</div>
                        <div class="fw-bold text-dark" style="font-size: 0.95rem;"><?= htmlspecialchars($fh['LONGNAME'] ?: $fh['NAME']) ?></div>
                        <div class="d-flex justify-content-between align-items-center mt-1">
                            <span class="small text-muted fw-bold"><i class="bi bi-geo-alt me-1"></i><?= htmlspecialchars($fh['NAME']) ?></span>
                        </div>
                        <?php if(!empty($fh['DESCRIPTION'])): ?>
                            <p class="small text-muted mt-2 mb-0" style="line-height: 1.3;"><?= htmlspecialchars($fh['DESCRIPTION']) ?></p>
                        <?php endif; ?>
                    </div>
                    <?php endforeach; ?>
                </div>
...
function focusLocation(placeName) {
